...
$this->form->birthplace = getFromRequest('birthplace',true,'Błędne wywołanie aplikacji');
		...
		if (empty(trim($this->form->birthdate))) {
			getMessages()->addError('Wprowadź datę urodzenia');
                ...
                if (empty(trim($this->form->birthplace))) {
			getMessages()->addError('Wprowadź miejsce urodzenia');
		}
		if ( getMessages()->isError() ) return false;
